...
__unused static float const kMTLTextureCoordinatesIdentity[8] = {
    0.0, 1.0,  //
    0.0, 0.0,  //
    1.0, 1.0,  //
    1.0, 0.0   //
...
__unused static void replaceArrayElements(float arr0[], float arr1[], int size) {
...
//倒N形
__unused static void genMTLVertices(CGRect rect, CGSize containerSize, float vertices[16], BOOL reverse, BOOL normalized) {
    if (vertices == NULL) {
...
__unused static simd_float4x4 getMetalMatrixFromGLKMatrix(GLKMatrix4 matrix) {
...
/// @param size 屏幕大小
/// @return 归一化坐标
__unused static CGPoint dc_to_ndc(CGPoint pt_dc, CGSize size) {
...
/// @param size 屏幕大小
/// @return 屏幕坐标
__unused static CGPoint ndc_to_dc(CGPoint pt_ndc, CGSize size) {

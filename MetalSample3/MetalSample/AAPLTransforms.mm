...
void AAPL::genQuadVertices(simd::float4 result[4], const simd::float4 targetRect, const simd::float2 containerSize, bool reverse, bool normalized) {
    if (result == nullptr) {
        std::cout << "AAPL::genVertices params illegal. \n";
        assert(0);
        return;
    }

    if (containerSize.x <= 0 || containerSize.y <= 0) {
        std::cout << "generateMTLVertices params containerSize illegal. \n";
        assert(0);
        return;
    }
    ...
    if (normalized) {
        originX = -1.0 + 2.0 * targetRect.x / containerSize.x;
        originY = 1.0 - 2.0 * targetRect.y / containerSize.y;
        width = 2.0 * targetRect.z / containerSize.x;
        height = 2.0 * targetRect.w / containerSize.y;
    } else {
    ...
    if (reverse) {
        if (normalized) {
            result[0] = simd::make_float4(originX, originY - height, 0.0, 1.0);
            result[1] = simd::make_float4(originX, originY, 0.0, 1.0);
            result[2] = simd::make_float4(originX + width, originY - height, 0.0, 1.0);
            result[3] = simd::make_float4(originX + width, originY, 0.0, 1.0);
            return;
        }
        result[0] = simd::make_float4(originX, originY + height, 0.0, 1.0);
        result[1] = simd::make_float4(originX, originY, 0.0, 1.0);
    ...
    if (normalized) {
        result[0] = simd::make_float4(originX, originY, 0.0, 1.0);
        result[1] = simd::make_float4(originX, originY - height, 0.0, 1.0);
        result[2] = simd::make_float4(originX + width, originY, 0.0, 1.0);
...
SSVertex AAPL::makeVertex(simd::float4 position, simd::float2 textureCoordinate) {
    SSVertex s;
    s.position = position;
    s.textureCoordinate = textureCoordinate;
    return s;
}
